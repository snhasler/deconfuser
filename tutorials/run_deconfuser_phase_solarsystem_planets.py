...
output_file = f"medi_SS_VEMJ_output_tol{tolerances[0]}_{now}.txt"
run_log =  f"run_log_medi_SS_VEMJ_tol{tolerances[0]}_{now}.log"
...
# ------------------------------------------------------------
# Read in systems
df_system_params = pd.read_csv(systems_orb_params_file, header=0, delimiter=', ')
...
# system = phot.System(n_exozodi=0, n_leakage=0, n_zodi=0) # background count contributions in units of s^-1
...
detector = phot.Detector(qe=0.837, cic=0.016, dark_current=1.39e-5, read_noise=120, gain=1000, 
                    fwc=10000000, conversion_gain=1.0, t=3600, D=2.36, throughput=0.38, f_pa=0.039,
                    wavelength=573.8e-9, bandwidth=56.5e-9) 
...
for _ in range(n_systems):
    ...
    i_vals = np.array(system_data['i'].values) + np.radians(61)
    o_vals = np.array(system_data['o'].values)
    O_vals = np.array(system_data['O'].values)
    M0_vals = np.array(system_data['M0'].values) + np.radians(np.random.uniform(0, 2*np.pi))
    print('a_valS: ', a_vals)
    # Set up planets
    radii = [6051.8e3, 6371e3, 3389.5e3, 69911e3] # radii of planets in m
    Ags = [0.67, 0.3, 0.17, 0.54]
    for k in range(n_planets):
        ...
        O = O_vals[k]
        M0 = M0_vals[k]
        planet = phot.Planet(R_p=radii[k], Ag=Ags[k])
        planet.add_orb_params(a, e, i, o, O, M0)
